  ...
  try {
    ...
    for (const pharmacy of pharmacies) {
      ...
      if (address) {
        ...
        let distance = findDistanceBetween(
          latitude,
          longitude,
          address.Addr_Latitude,
          address.Addr_Longitude
        );

        // Get Operation Time of the pharmacy
        let operationTimes = await optimeService.getOptByPharmacyId(
          pharmacy.ID
        );
...
export const getPharmacyByName = async (pharName, authorizationToken) => {
  let query = "SELECT * FROM inventory.Pharmacy WHERE Pcy_Name = ? ;";

  try {
    let pharmacy = await connection.promise().execute(query, [pharName]);
    pharmacy = pharmacy[0][0];
...
export const getPharmacyByNameAndDistance = async (
  latitude,
  longitude,
  pharName,
  authorizationToken
  ...
  try {
    ...
    throw new Error(`Get Pharmacy By Name and Distance: ${error.message}`);
  }
};
